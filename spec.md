...
- **OF6.** Where a configuration file exists at `$TRY_CONFIG`, `$XDG_CONFIG_HOME/tryit/config.toml`, or `~/.config/tryit/config.toml`, the system shall read the `theme` and `animation` settings from it, with precedence environment > file > default. A missing, unreadable or malformed file shall yield defaults rather than preventing startup. (Added 2026-07-18. Partially addresses backlog B1; the remaining `try-rs` config keys are still unimplemented.) (Added 2026-07-18.)
