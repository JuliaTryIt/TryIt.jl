...
Resolution order (FR-001 / UB1):
 ...
 2. `ENV["TRY_PATH"]`.
 3. `tries_path` in the configuration file.
 4. `\$HOME/work/tries` (default), matching `try-rs`.

Environment over file over default is the precedence every other
setting follows; see [`_setting`](@ref).
...
struct TriesPath
    ...
    root::String
    """
    Where the path came from: `:arg`, `:env`, `:config`, or `:default`.
    """
    source::Symbol
end
...
function TriesPath(;
    ...
    root, source = _resolve_tries_root(positional, config)
...
function _resolve_tries_root(
    ...
    from_file = get(read_config(config), "tries_path", "")
    if from_file isa AbstractString && !isempty(strip(from_file))
        return (_expand_home(strip(from_file)), :config)
    end
...
`Base.expanduser` consults the real home directory rather than the
`HOME` variable, which would make this untestable and would ignore a
deliberate override. try-rs writes `tries_path = "~/src/tries"` in its
own config, so this spelling is what anyone aligning the two tools by
hand will copy.
"""
function _expand_home(path::AbstractString)
    path == "~" || startswith(path, "~/") || return String(path)
    home = get(ENV, "HOME", homedir())
    return path == "~" ? String(home) : joinpath(home, path[3:end])
end
