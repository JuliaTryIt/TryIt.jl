"""
Tachikoma model backing the try selector.

A thin wrapper over [`SelectorState`](@ref), which holds everything
that is not Tachikoma's. Only two fields are irreducibly a frontend's:
the terminal handle and the resolved background object. Everything
else is reached by forwarding, so `m.filter` and `m.state.filter` name
the same slot and no call site had to learn where a field lives.

EARS coverage: ED1, ED2, ED3, ED4, ED12, SD1, UN7.
"""
mutable struct SelectorSession <: Tachikoma.Model
    """
    ...
    the model.
    """
    terminal::Union{Nothing, Tachikoma.Terminal}
    """
    Animated background, or `nothing` when opted out. Resolved once
    at open — see [`background_from_env`](@ref). Its type spans both
    layers (`ColorBackground` is core, `Tachikoma.Background` is
    not), so it stays on this side of the boundary.
    """
    background::Union{Nothing, SelectorBackground}
end

function SelectorSession(; root::TriesPath)
    SelectorSession(
        SelectorState(root=root), nothing, nothing)
end

# Fields this wrapper actually owns; everything else lives on `state`.
const _SESSION_FIELDS = (:state, :terminal, :background)

function Base.getproperty(m::SelectorSession, name::Symbol)
    name in _SESSION_FIELDS && return getfield(m, name)
    return getproperty(getfield(m, :state), name)
end

function Base.setproperty!(m::SelectorSession, name::Symbol, x)
    name in _SESSION_FIELDS && return setfield!(m, name, x)
    return setproperty!(getfield(m, :state), name, x)
end

Base.propertynames(m::SelectorSession) = (
    _SESSION_FIELDS..., fieldnames(SelectorState)...)
...
const OPACITY_NAMES = ["0%", "10%", "25%", "50%", "75%", "90%", "100%"]
const OPACITY_VALUES = [0.0, 0.1, 0.25, 0.5, 0.75, 0.9, 1.0]

function _open_opacity_picker!(m::SelectorSession)
    m.background === nothing && return nothing
    m.mode = :opacity
    if hasproperty(m.background, :intensity)
        m.opacity_before = m.background.intensity
        # Find closest predefined value
        dist, idx = findmin(abs.(OPACITY_VALUES .- m.opacity_before))
        m.opacity_index = idx
    else
        m.opacity_before = 0.5
        m.opacity_index = 4
    end
    return nothing
end

function _update_opacity!(m::SelectorSession, evt::Tachikoma.KeyEvent)
    key = evt.key
    if key === :enter
        m.mode = :normal
    elseif key === :escape
        m.background = with_intensity(m.background, m.opacity_before)
        m.mode = :normal
    elseif key === :up || key === :left
        m.opacity_index = max(1, m.opacity_index - 1)
        m.background = with_intensity(m.background, OPACITY_VALUES[m.opacity_index])
    elseif key === :down || key === :right
        m.opacity_index = min(length(OPACITY_NAMES), m.opacity_index + 1)
        m.background = with_intensity(m.background, OPACITY_VALUES[m.opacity_index])
    end
    return nothing
end
...
function Tachikoma.update!(m::SelectorSession, evt::Tachikoma.KeyEvent)
    ...
    elseif key === :ctrl && evt.char == 'o'
        _open_opacity_picker!(m)
    elseif key === :ctrl && evt.char == 'w'
        # ED22 — persist theme and animation, reporting where. Reading
        # the active theme is the view layer's job: `save_settings` is
        # core and cannot reach Tachikoma.
        notify!(m, save_settings(
            Tachikoma.theme().name, animation_name(m.background),
            hasproperty(m.background, :intensity) ? m.background.intensity : 0.25
        ))
...
function _create_from_filter!(m::SelectorSession)
    ...
    # A pasted URL is not a name. Slugifying one produced an empty
    # hundred-character directory that the caller was then sent into;
    # it is routed by the same rule the command line uses (ED27).
    #
    # Only the decision is taken here. The transfer itself runs after
    # the event loop exits: a network round trip inside the reducer
    # would freeze the display, and UB5's selector exception means a
    # diagnostic written from a key handler reaches nobody.
    kind = url_kind(raw)
    if kind === :clone || kind === :fetch
        m.exit_action = kind
        m.exit_url = String(raw)
        m.done = true
        return nothing
    end
    try
...
function Tachikoma.view(m::SelectorSession, f::Tachikoma.Frame)
    ...
    m.mode === :opacity && _render_opacity_picker(m, buf, f.area)
    ...
    m.show_fps && _render_fps(m, buf, f.area)
    return nothing
end
...
Advances the meter with wall-clock time — one sample per rendered frame
— and prints the rolling average. Drawn last so it sits above every
panel; kept to the top-right, clear of the search caret and the
`page x/y` counter on the folders header.
"""
function _render_fps(m::SelectorSession, buf, area)
    value = fps_tick!(m.fps, time())
    label = string(round(value, digits=1), " fps")
    x = area.x + area.width - length(label) - 1
    x < area.x && return nothing
    Tachikoma.set_string!(buf, x, area.y, label, Tachikoma.tstyle(:accent, bold=true))
    return nothing
end
...
function _render_theme_picker(m::SelectorSession, buf, area)
    ...
    items = Tachikoma.ListItem[Tachikoma.ListItem(names[i], Tachikoma.tstyle(:text))
...
"""
Draw the opacity picker (Ctrl-O).
"""
function _render_opacity_picker(m::SelectorSession, buf, area)
    names = OPACITY_NAMES
    height = min(length(names) + 2, max(5, area.height - 2))
    width = min(area.width - 4, 30)
    (width < 14 || height < 5) && return nothing
    ...
    Tachikoma.render(
        Tachikoma.SelectableList(
            [Tachikoma.ListItem(n, Tachikoma.tstyle(:text)) for n in names];
            selected=m.opacity_index,
            focused=true,
            block=Tachikoma.Block(
                title="Opacity",
                title_right=string(m.opacity_index, "/", length(names)),
                title_style=Tachikoma.tstyle(:title, bold=true),
                border_style=Tachikoma.tstyle(:accent, bold=true)
            ),
            highlight_style=Tachikoma.tstyle(:accent, bold=true)
...
function _render_about(m::SelectorSession, buf, area)
    lines = [
        ...
        ("Repo: github.com/s-celles/TryIt.jl", Tachikoma.tstyle(:text)),
        ("Docs: s-celles.github.io/TryIt.jl", Tachikoma.tstyle(:text)),
