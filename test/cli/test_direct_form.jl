...
@testitem "cli: init emits a shell function (ED13)" begin
    ...
    # Pins no tries path when no arg was given: resolution (env, then
    # `tries_path` in the config file, then the default) belongs to
    # _resolve_tries_root. Pinning it here made the environment branch
    # always win and the config setting unreachable.
