...
@testitem "spec: the documented default tries root matches the code" begin
    ...
    # An explicit, non-existent config path: `_resolve_tries_root` now
    # consults `tries_path` in the config file, and a developer whose
    # own config sets it would otherwise see this resolve to *their*
    # root while CI, having no config, still saw the default.
    no_config = joinpath(mktempdir(), "absent.toml")
    default_root = withenv("TRY_PATH" => nothing, "HOME" => "/home/u") do
        TryIt._resolve_tries_root(nothing, no_config)[1]
    end
    ...
    # The emitted function must NOT name the default. It used to pin
    # `TRY_PATH=${TRY_PATH:-$HOME/work/tries}`, which always set the
    # variable and so made the environment branch of the resolution
    # win every time — leaving `tries_path` in the config file
    # unreachable through the only entry point anyone actually uses.
    # The fallback now lives solely in `_resolve_tries_root`.
    io = IOBuffer()
    TryIt.emit_shell_init(io, nothing)
    emitted = String(take!(io))
    @test !occursin(tail, emitted)
    @test !occursin("TRY_PATH=", emitted)
end
