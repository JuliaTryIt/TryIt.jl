...
@testitem "errors: FAILURE is 1 and distinct from NOT_FOUND (UB6, UN11)" begin
    using TryIt: ExitCode

    # `fetch` needs a "the operation failed" that is not a usage
    # error, not a permission error, and not a missing dependency.
    # Overloading 127 for an HTTP 404 would give scripts a status
    # code meaning two different things.
    @test Int(ExitCode.FAILURE) == 1
    @test ExitCode.FAILURE !== ExitCode.NOT_FOUND
end
...
@testitem "errors: exit codes do not leak into the TryIt namespace" begin
    ...
    for name in (:SUCCESS, :FAILURE, :PERMISSION, :USAGE, :NOT_FOUND, :SIGINT,
        :EXIT_SUCCESS, :EXIT_FAILURE, :EXIT_PERMISSION, :EXIT_USAGE,
