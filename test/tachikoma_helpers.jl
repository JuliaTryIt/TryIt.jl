...
`clone_stderr` is written verbatim to the stub's stderr on the
`clone` path. Real git always emits its `Cloning into '…'` banner
there *before* any error, so a test that cares which line TryIt
surfaces (UN3) has to reproduce the multi-line shape rather than a
single error string.
...
function _write_git_stub(
    ...
    # Carried in a sidecar file rather than interpolated into the
    # script: the payload is multi-line and contains quotes and URLs,
    # and escaping it through both a Julia string and a shell heredoc
    # is exactly the kind of thing that silently truncates.
    stderr_file = joinpath(dirname(path), "clone_stderr.txt")
    isempty(clone_stderr) || write(stderr_file, clone_stderr)
    if is_windows
        ...
        if exist "%~dp0clone_stderr.txt" type "%~dp0clone_stderr.txt" 1>&2
        ...
        """ * ((clone_exit >= 0) ? "    mkdir -p \"\$4\"\n" : "") * """
            if [ -f "\$(dirname "\$0")/clone_stderr.txt" ]; then
              cat "\$(dirname "\$0")/clone_stderr.txt" >&2
            fi
            exit $(abs(clone_exit))
            ;;
          worktree)
